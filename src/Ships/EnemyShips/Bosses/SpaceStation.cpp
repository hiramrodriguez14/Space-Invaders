...
SpaceStation::SpaceStation(int xpos, int ypos, string name) : Boss(ofGetWidth()/2 - 60, ofGetHeight()/2 - 60   ,0.5, 250, name) {
    ...
    enemyHitBox = new HitBox(ofGetWidth()/2-50, ofGetHeight()/2-40, enemyShipSprite.getWidth() * 0.3, enemyShipSprite.getHeight() * 0.38);
    
    shootingPoint = ofPoint(pos.x + enemyHitBox->box.getWidth()/2-20 ,  enemyHitBox->box.getHeight() / 2 + 300 );
